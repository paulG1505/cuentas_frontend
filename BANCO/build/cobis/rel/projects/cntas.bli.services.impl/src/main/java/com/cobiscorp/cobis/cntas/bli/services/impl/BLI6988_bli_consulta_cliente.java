...
import com.cobiscorp.cobis.cntas.model.FiltroCliente;
...
@Properties({
  ...
  @Property(name = "bli.id", value = "BLI6988_bli_consulta_cliente")
})
public class BLI6988_bli_consulta_cliente implements IBLIExecutor {
  private static final ILogger logger = LogFactory.getLogger(BLI6988_bli_consulta_cliente.class);
  ...
  public void execute(DynamicRequest dynamicRequest) {
    ...
    try {
      ...
      DataEntity rowFiltroCliente;
      if (row != null && FiltroCliente.ENTITY_NAME.equals(row.getEntityName())) {
        rowFiltroCliente = row;
      } else {
        rowFiltroCliente = BLIUtils.getDataEntity(dynamicRequest, FiltroCliente.ENTITY_NAME);
        if (rowFiltroCliente == null) {
          throw new DesignerRuntimeException("No existe entidad " + FiltroCliente.ENTITY_NAME);
        }
      }

      String wcedula = rowFiltroCliente.get(FiltroCliente.CEDULA);
